...
function throwFireball() {
    ...
    const fireballScale = window.themeConfig?.mode === 'custom'
        ? screenHeight / 1200
        : screenHeight / 345;

    let fireball = this.physics.add.sprite(player.getBounds().x + (player.width * 1.15), player.getBounds().y + (player.height / 1.25), 'fireball').setScale(fireballScale);
...
function fireballCollides(fireball, entitie) {
    ...
    if (entitie.enemyType === 'mud-boulder') {
        entitie.anims.play(entitie.hurtAnimationKey || 'mud-boulder-hurt', true);
        if (typeof showInteractionLabel === 'function') {
            showInteractionLabel.call(this, entitie, '\u5de5\u7a0b\u6574\u6cbb', '#d7ecff');
        }
        entitie.dead = true;
        entitie.body.enable = false;
        this.goombasGroup.remove(entitie);
        entitie.setVelocity(0, 0);
    ...
    if (entitie.enemyType === 'wandering-citizen') {
        startCitizenEvacuation.call(this, entitie);
        return;
    }
