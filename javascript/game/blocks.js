...
function revealHiddenBlock(player, block) {
    ...
    if (reward === 'coin') {
    ...
    } else if (reward === 'super-mushroom') {
        this.powerUpAppearsSound.play();
        let mushroom = this.physics.add.sprite(block.getBounds().x, block.getBounds().y, 'super-mushroom').setScale(getPowerUpScale()).setOrigin(0).setBounce(1, 0);
        this.tweens.add({
            targets: mushroom,
            duration: 300,
            start: performance.now(),
        ...
        this.powerUpAppearsSound.play();
        let fireFlower = this.physics.add.sprite(block.getBounds().x, block.getBounds().y, 'fire-flower').setScale(getPowerUpScale()).setOrigin(0);
